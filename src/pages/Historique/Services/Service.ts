...
export const historiqueService = {
  ...
  async fetchPage(
    ...
    return api.get(`api/paiements/historique/?${filtresEnParams(filtres, page, pageSize)}`, { silentLoading: true, silentSuccess: true } as any);
  },
  ...
  async exporterPDF(filtres: Partial<FiltresState>, page = 1, pageSize = 10): Promise<void> {
    const response = await axiosBlob.get(
      `api/paiements/historique/export-pdf/?${filtresEnParams(filtres, page, pageSize)}`
    );
    downloadBlob(response.data, `historique-${Date.now()}.pdf`);
  },
