...
const TableauEnAttente: React.FC<Props> = ({ data, page, onPageChange, onConfirmerRH }) => {
  ...
  const columns = useMemo(() => [
    ...
    col.accessor(r => r.employe?.nom_complet, {
      ...
      cell: ({ row }) => (
        <button
          className="btn btn-link text-primary fw-medium p-0 text-start"
    ...
    col.display({
      ...
      cell: ({ row }) => (
        <button
          className="btn btn-soft-warning btn-sm d-flex align-items-center gap-1"
          onClick={() => setModal(row.original)}
        >
  ...
  const selectedMontant = data.data
        ...
        <Card className="card-height-100">
          <CardHeader className="d-flex align-items-center flex-wrap gap-2 border-bottom">
            ...
            <div className="d-flex align-items-center gap-2 flex-grow-1">
              ...
              <Badge color="warning" className="bg-warning-subtle text-warning">{data.total}</Badge>
            </div>

            {/* ── Barre action groupée — slide-in dès qu'une ligne est cochée ── */}
