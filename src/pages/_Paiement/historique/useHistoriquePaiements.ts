...
import { PAGE_LIMIT, chipToDateRange }                  from './historique.constants';
...
export interface UseHistoriquePaiementsReturn {
  ...
  // Tri — OnChangeFn est le type exact attendu par useReactTable
  ...
  handleExportPDF: () => Promise<void>;       // exporte selon filtres actifs
...
export const useHistoriquePaiements = (): UseHistoriquePaiementsReturn => {
  ...
  // ── OnChangeFn<SortingState> : setState de React est directement compatible ─
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      d = d.filter(l =>
        l.employe_nom.toLowerCase().includes(q) ||
        l.employe_id.toLowerCase().includes(q)
      );
    }
  ...
  const handleExportCSV = async () => {
    try {
      await historiqueService.exporterCSV(filtered);
  ...
  /** Exporte le PDF en transmettant les filtres actifs au backend. */
  const handleExportPDF = async () => {
    try {
      await historiqueService.exporterPDF({ search, dept, dateRange, activeChip, page: safePage });
  ...
  return {
    ...
    // setSorting est Dispatch<SetStateAction<SortingState>>, compatible OnChangeFn<SortingState>
    sorting,
    ...
    page: safePage, totalPages, setPage: setPageRaw,
